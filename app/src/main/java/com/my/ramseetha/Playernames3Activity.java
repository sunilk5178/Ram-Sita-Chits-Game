...
public class Playernames3Activity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.playernames3);
		initialize();
		initializeLogic();
	}
	...
	private void  initialize() {
		...
		button1.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View _v) { 
				if ((edittext1.getText().toString().length() > 0) && ((edittext2.getText().toString().length() > 0) && (edittext3.getText().toString().length() > 0))) {
					p13.edit().putString("x1", edittext1.getText().toString()).commit();
					p13.edit().putString("x2", edittext2.getText().toString()).commit();
					p13.edit().putString("x3", edittext3.getText().toString()).commit();
					x.setAction(Intent.ACTION_VIEW);
					x.setClass(getApplicationContext(), Game3Activity.class);
					startActivity(x);
					finish();
				}
				else {
		...
		button2.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _v) { 
				p13.edit().putString("x1", "Player1").commit();
				p13.edit().putString("x2", "Player2").commit();
				p13.edit().putString("x3", "Player3").commit();
				x.setAction(Intent.ACTION_VIEW);
				x.setClass(getApplicationContext(), Game3Activity.class);
				startActivity(x);
				sound = sp.play((int)(sid), 1.0f, 1.0f, 1, (int)(0), 1.0f);
